...
def user():
    ...
    return CustomUser.objects.create_user(
        email=f"test_{unique_id}@example.com", password="testpass123", username=f"testuser_{unique_id}", is_active=True
    )
...
@pytest.fixture
def photo_coordinate(user, season, scene, taste, sample_image):  # userを直接注入
    """テスト用写真コーディネートを作成"""
    coord = PhotoCoordinate.objects.create(user=user, image=sample_image)  # userを直接使用
...
@pytest.fixture
def custom_coordinate(user, season, scene, taste, sample_image):  # userを直接注入
    """テスト用カスタムコーディネートを作成"""
    coord = CustomCoordinate.objects.create(
        user=user,  # userを直接使用
        image=sample_image,
        background="bg-white",
    )
...
@pytest.fixture
def fashion_items(user, subcategory, brand, season, design, color, price_range, sample_image):  # userを直接注入
    """テスト用ファッションアイテムを2つ作成"""
    items = []

    # 1つ目のアイテム
    item1 = FashionItem.objects.create(
        user=user,  # auth_clientからではなく直接userを使用
        sub_category=subcategory,
        brand=brand,
        price_range=price_range,
        design=design,
